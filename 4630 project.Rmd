...
The multiple linear regression model achieved an RMSE of 0.464 on log(price) and an R² of 0.519. This means the model explains about 52% of the variability in log-transformed Airbnb prices
...
rf_fit <- randomForest(
  ...
  mtry  = floor(sqrt(ncol(train) - 1)),          # vars tried at each split
  importance = TRUE
)
...
rf_metrics <- get_metrics(test$log_price, rf_pred)
rf_metrics
```
...
```{r}
importance(rf_fit)
varImpPlot(rf_fit, n.var = 9, main = "Random Forest Variable Importance")
```
